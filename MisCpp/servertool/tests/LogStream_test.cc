...
{
  basetool::LogStream os;
  const basetool::LogStream::Buffer& buf = os.buffer();
...
BOOST_AUTO_TEST_CASE(testLogStreamFmts)
{
  ...
  os << basetool::Fmt("%4d", 1);
  BOOST_CHECK_EQUAL(buf.toString(), string("   1"));
  os.resetBuffer();

  os << basetool::Fmt("%4.2f", 1.2);
  BOOST_CHECK_EQUAL(buf.toString(), string("1.20"));
  os.resetBuffer();

  os << basetool::Fmt("%4.2f", 1.2) << basetool::Fmt("%4d", 43);

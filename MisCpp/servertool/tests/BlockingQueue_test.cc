 ...
 public:
      ...
      threads_(numThreads)
  {
    for (int i = 0; i < numThreads; ++i)
    {
      ...
      threads_.push_back(new basetool::Thread(
            boost::bind(&Test::threadFunc, this), basetool::string(name)));
    }
  ...
  void run(int times)
  {
    ...
    for (int i = 0; i < times; ++i)
    {
      ...
      queue_.put(buf);
      printf("tid=%d, put data = %s, size = %zd\n", basetool::CurrentThread::tid(), buf, queue_.size());
    }
  }
 ...
 private:
  ...
  void threadFunc()
  {
    ...
    while (running)
    {
      ...
      printf("tid=%d, get data = %s, size = %zd\n", basetool::CurrentThread::tid(), d.c_str(), queue_.size());
      running = (d != "stop");
    }
  ...
  basetool::BlockingQueue<std::string> queue_;
...
// std::unique_ptr requires gcc 4.4 or later
  ...
  basetool::BlockingQueue<std::unique_ptr<int>> queue;

...
int main()
{
  ...
  {
  basetool::GzipFile writer = basetool::GzipFile::openForAppend(filename);
  if (writer.valid())
  {
  ...
  basetool::GzipFile reader = basetool::GzipFile::openForRead(filename);
  if (reader.valid())
  {
    char buf[256];
  ...
  {
  basetool::GzipFile writer = basetool::GzipFile::openForWriteExclusive(filename);

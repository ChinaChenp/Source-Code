...
void forkBench()
{
  ...
  double timeUsed = timeDifference(basetool::Timestamp::now(), start);
...
int main(int argc, char* argv[])
{
  ...
  {
    basetool::Timestamp now(basetool::Timestamp::now());
    basetool::Thread t2(boost::bind(threadFunc2, now));
    t2.start();
    t2.join();
  }
  {
